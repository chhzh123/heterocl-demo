...
def RPReLU(data, x0, y0, beta, name="rprelu", dtype=None):
    ...
    return hcl.compute(data.shape, lambda nn, cc, hh, ww:
                        hcl.select(data[nn,cc,hh,ww] + x0[cc] > 0,
                        data[nn,cc,hh,ww] + x0[cc],
                        beta[cc] * (data[nn,cc,hh,ww] + x0[cc])) + y0[cc],
...
# hcl_out = hcl.asarray(np.zeros((10,64,8,8)).astype(np.float),dtype=qtype_float)
